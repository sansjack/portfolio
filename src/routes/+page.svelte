...
>
        ...
        <div class="w-full my-4">
            ...
            >
                {#each jobExperiences.sort((a, b) => b.startDate - a.startDate) as job}
                    ...
                    >
                        <div
                            class="flex flex-row items-center gap-2 my-1 group text-sm"
                        >
                            <a
                                href={job.href}
                                target="_blank"
    ...
    <div class="w-full flex items-center justify-center">
        ...
        >
            <div class="flex flex-col items-center justify-center">
                <h3 class="justify-start self-start text-center">
                    Writeups ✏️
                </h3>
            ...
            <div class="w-full my-4">
                ...
                <ul class="relative w-full space-y-4 gap-8 sm:space-y-0 my-2">
                    {#each projects.sort((a, b) => Number(a.archived) - Number(b.archived) || b.date - a.date) as project}
                        <li
                            class="flex flex-wrap items-center justify-between hover:bg-text -mx-2 px-2 gap-x-2 gap-y-0"
                        >
                            <div
                                class="flex flex-row items-center gap-2 my-1 group tooltip min-w-0"
                                data-tip={project.description}
                            >
                                <project.Icon
                                    class="relative size-7 shrink-0 grayscale group-hover:grayscale-0"
                                />

                                <a
                                    target="_blank"
                                    href={project.link}
                                    class="link hover:link-primary font-bold truncate"
                                >
                                    {project.title}</a
                                >
                                {#if project.archived}
                                    <span
                                        class="badge badge-ghost text-xs shrink-0"
                                        >Archived</span
                                    >
                                {/if}
                            </div>
                            <p class="text-xs shrink-0">
